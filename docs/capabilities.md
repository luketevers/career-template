...
| `python3 engine/boards.py URL-or-source [--titles-from profile.yaml] [--titles a,b] [--query Q] [--role R] [--location L] [--max N] [--json] [--list]` | Fetch a job board into one normalized posting shape. Detects the ATS from the URL (or a careers page embedding one); aggregators by name (`yc`, `hn`, …). Title filters ignore case, hyphens, and spaces (`Full-Stack Engineer` matches `Fullstack Engineer`). Exit 1 = nothing matched, 2 = unsupported source. |
